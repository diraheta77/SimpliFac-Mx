...
namespace simplifile.Helpers
{
    public static class OpenSslHelper
    {
        ...
        public static dynamic CertificateData(string pCerFile)
        {
            using (X509Certificate2 pubOnly = new X509Certificate2(pCerFile))
            {
                var rfc = "";
                var fields = pubOnly.Subject.Split(',');
                foreach (var f in fields)
                {
                    if (f.Contains("OID.2.5.4.45"))
                    {
                        rfc = f.Split('=')[1];
                        if (rfc.Contains("/"))
                        {
                ...
                return  new
                {
                    InitDate = pubOnly.NotBefore,
                    FinalDate = pubOnly.NotAfter,
                    Rfc = rfc
                };
            }
        }

        public static bool ValidKeyPassword(string keyFile,string password)
        {
            var key = OpenKeyFile(keyFile, password);
            return key != null;
        }

        public static bool ValidCerKeyPassword(string cerFile,string keyFile, string password)
        {
            try
            {
                var key = OpenKeyFile(keyFile, password);
                if (key != null)
                {
                    using (X509Certificate2 pubOnly = new X509Certificate2(cerFile))
                    {
                        using (X509Certificate2 pubPrivEphemeral = pubOnly.CopyWithPrivateKey(key)) ;
                    }
                    return true;
                }
                return false;
